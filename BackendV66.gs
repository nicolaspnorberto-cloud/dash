...
/* =========================================================
   ...
   O dashboard solicita ao Apps Script uma sincronização real
   da Matinal/LM + GEROT. O Web App apenas agenda a execução
   e retorna rápido; o navegador aguarda o Blob receber os
   novos dados.
========================================================= */

const V66_FORCE_LM_DAYS = 3;
const V66_FORCE_CHUNK_SIZE = 1000;
 ...
 * - a Vercel envia EMAIL_WEBHOOK_TOKEN;
 * - aqui validamos contra WEBHOOK_TOKEN já existente no Script Properties.
 */
function doPost(e) {
  try {
    ...
    const expected = webhookTokenV6_();
    const received = String(body.token || '').trim();

    if (!expected || !received || received !== expected) {
      return respostaJsonV66_({
        ok: false,
    ...
    const currentStatus = String(
      props.getProperty('V66_REFRESH_STATUS') || ''
    ).toUpperCase();
    const currentRequestedAt = props.getProperty('V66_REFRESH_REQUESTED_AT') || '';
    const currentMs = Date.parse(currentRequestedAt);

    // Evita empilhar cliques enquanto uma sincronização recente já está em curso.
    if (
      (currentStatus === 'REQUESTED' || currentStatus === 'RUNNING') &&
      Number.isFinite(currentMs) &&
      (now.getTime() - currentMs) < 5 * 60 * 1000
    ) {
      return respostaJsonV66_({
        ok: true,
        accepted: true,
        alreadyRunning: true,
        requestId: props.getProperty('V66_REFRESH_REQUEST_ID') || '',
    ...
    props.setProperties({
      V66_REFRESH_STATUS: 'REQUESTED',
      V66_REFRESH_REQUEST_ID: requestId,
      V66_REFRESH_REQUESTED_AT: requestedAt,
      V66_REFRESH_ACTION: action,
      V66_REFRESH_ERROR: ''
    });

    ScriptApp.newTrigger('executarAtualizacaoAgoraV66')
      .timeBased()
      .after(1000)
      .create();
...
function respostaJsonV66_(data) {
...
/**
 * Executada nos servidores do Google após o clique.
 * As fontes são independentes: erro de uma não bloqueia a outra.
 */
function executarAtualizacaoAgoraV66() {
  const props = PropertiesService.getScriptProperties();
  const action = String(
    props.getProperty('V66_REFRESH_ACTION') || 'all'
  ).toLowerCase();

  props.setProperties({
    V66_REFRESH_STATUS: 'RUNNING',
    V66_REFRESH_STARTED_AT: new Date().toISOString()
  });
  ...
  const finalStatus = result.errors.length ? 'PARTIAL' : 'DONE';

  props.setProperties({
    V66_REFRESH_STATUS: finalStatus,
    V66_REFRESH_FINISHED_AT: new Date().toISOString(),
    V66_REFRESH_ERROR: result.errors.join(' | '),
    V66_REFRESH_RESULT: JSON.stringify(result)
  });

  Logger.log('V6.6 Atualizar agora: ' + JSON.stringify(result));
  return result;
}

/**
 * Sincronização rápida para o botão.
 * Varre a LM, mas envia somente os últimos 3 dias.
 * Assim inclui hoje/ontem sem reenviar 45 dias a cada clique.
 */
function sincronizarHCeMisscanAgoraV66() {
  validarConfiguracaoV6_();
  ...
  try {
    ...
    if (!rows.length) {
      ...
      for (
        let i = 0;
        i < rows.length;
        i += V66_FORCE_CHUNK_SIZE
      ) {
        chunks.push(
          rows.slice(i, i + V66_FORCE_CHUNK_SIZE)
        );
      }
    }
    ...
    chunks.forEach(function(chunk, index) {
      ...
      const payload = {
        ...
        meta: {
          ...
          misscanSourceLastRow: misscanResult.lastRow,
          sourceRowsScanned: misscanResult.sourceRowsScanned,
          days: V66_FORCE_LM_DAYS,
          periodStart: misscanResult.periodStart,
          periodEnd: misscanResult.periodEnd,
          periodLabel: misscanResult.periodLabel,
          source: 'Apps Script Push V6.6 • Atualizar agora',
          syncMode: 'FORCE_REFRESH_CHUNKED',
    ...
    Logger.log(
      'V6.6 LM AGORA OK: ' +
      rows.length + ' Misscans dos últimos ' +
      V66_FORCE_LM_DAYS + ' dias em ' +
      chunks.length + ' lote(s).'
    );
...
function statusAtualizacaoAgoraV66() {
  const props = PropertiesService.getScriptProperties();
  const status = {
    status: props.getProperty('V66_REFRESH_STATUS') || '',
    requestId: props.getProperty('V66_REFRESH_REQUEST_ID') || '',
    requestedAt: props.getProperty('V66_REFRESH_REQUESTED_AT') || '',
    startedAt: props.getProperty('V66_REFRESH_STARTED_AT') || '',
    finishedAt: props.getProperty('V66_REFRESH_FINISHED_AT') || '',
    error: props.getProperty('V66_REFRESH_ERROR') || '',
    result: props.getProperty('V66_REFRESH_RESULT') || ''
  };
  Logger.log(JSON.stringify(status, null, 2));
  return status;
}
