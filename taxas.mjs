...
function blockForTurn(value = '') {
  ...
  const blocks = [...new Set(turns.map(t => t === 'T4' ? 'T2' : t === 'T5' ? 'T3' : t)
...
function minDate(...values) {
  const dates = values.filter(v => /^\d{4}-\d{2}-\d{2}$/.test(String(v || ''))).sort();
  return dates[0] || '';
}

function maxDate(rows = []) {
  return rows.map(r => String(r?.date || '')).filter(v => /^\d{4}-\d{2}-\d{2}$/.test(v)).sort().at(-1) || '';
}

function emptyBlocks() {
  return { T1: { misscan: 0, volumeReal: 0 }, T2: { misscan: 0, volumeReal: 0 }, T3: { misscan: 0, volumeReal: 0 } };
}
...
export async function GET(request) {
  try {
    ...
    if (!gerot?.processed?.length) {
      return json({ ok: false, error: 'Produção GEROT ainda não sincronizada. Execute sincronizarGerotV65().' }, 503);
    }
    if (!histMeta) return json({ ok: false, error: 'Histórico Misscan ainda não sincronizado.' }, 503);

    const processedEnd = gerot.meta?.processedEnd || maxDate(gerot.processed);
    const missEnd = String(histMeta.historyEnd || '');
    const end = minDate(processedEnd, missEnd) || processedEnd || missEnd;
    ...
    for (const r of (gerot.processed || [])) {
      ...
      if (!block) continue;
      if (!processedDaily.has(r.date)) processedDaily.set(r.date, { week: r.week || '', blocks: { T1: 0, T2: 0, T3: 0 } });
    ...
    for (const month of months) {
      ...
      for (const row of (f.rows || [])) {
        ...
        missAllByDate.set(date, (missAllByDate.get(date) || 0) + 1);
        ...
        if (!block) { withoutHC++; continue; }
        if (!missBlockByDate.has(date)) missBlockByDate.set(date, { T1: 0, T2: 0, T3: 0 });
    ...
    const allDates = [...new Set([...processedDaily.keys(), ...missAllByDate.keys()])].sort();
    for (const date of allDates) {
      if (date < start || date > end) continue;
      const p = processedDaily.get(date);
      ...
      const blocks = {};
      let dayVolume = 0;
      const blockMiss = missBlockByDate.get(date) || { T1: 0, T2: 0, T3: 0 };

      for (const t of ['T1', 'T2', 'T3']) {
        const volumeReal = Number(p.blocks[t] || 0);
        const misscan = Number(blockMiss[t] || 0);
        const rate = volumeReal > 0 ? misscan / volumeReal * 100 : null;
        blocks[t] = { misscan, volumeReal, rate };
        if (volumeReal > 0) {
          sums[t].misscan += misscan;
          sums[t].volumeReal += volumeReal;
          dayVolume += volumeReal;
        }
      }
    ...
    for (const t of ['T1', 'T2', 'T3']) {
      rates[t] = {
        ...
        rate: sums[t].volumeReal > 0 ? sums[t].misscan / sums[t].volumeReal * 100 : null
      };
    }
    ...
    for (const d of daily) {
      ...
      if (!weeklyMap.has(key)) weeklyMap.set(key, { week: key, misscan: 0, volumeReal: 0, days: 0 });
    ...
    const weekly = [...weeklyMap.values()].map(w => ({
      ...w,
      rate: w.volumeReal > 0 ? w.misscan / w.volumeReal * 100 : null,
      target: TARGET
    }));

    const generalRate = general.volumeReal > 0 ? general.misscan / general.volumeReal * 100 : null;
    ...
    return json({
      ...
      general: {
        ...
        rate: generalRate,
        target: TARGET,
        gapPp: Number.isFinite(generalRate) ? generalRate - TARGET : null,
        unidentified,
        withoutHC,
        identifiedForBlocks,
    ...
    console.error('TAXAS_V65_ERROR', error);
    return json({ ok: false, error: error?.message || 'Falha ao calcular taxas V6.5.' }, 500);
  }
}
