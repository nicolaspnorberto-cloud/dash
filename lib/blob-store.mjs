...
async function mapLimit(items, limit, worker) {
  ...
  await Promise.all(
    Array.from({ length: Math.min(Math.max(1, limit), input.length) }, run)
  );
  return output;
}
...
export async function readHistoryRange(from, to, meta = {}) {
  ...
  if (meta?.storageGranularity === 'day-v1' || meta?.storageGranularity === 'hybrid-day-week-v1') {
    ...
    const dailyFiles = await mapLimit(daysToRead, 4, async day => ({
      day,
      file: await readJson(`misscan/history-days/${day}.json`, { rows: [] })
    }));

    for (const { day, file } of dailyFiles) {
      for (const row of (file?.rows || [])) {
        if (rowDateKey(row) === day) rows.push(row);
      }
    }
