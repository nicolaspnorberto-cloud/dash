...
const cacheable = /^misscan\/(?:hc|history-meta|gerot|producao-real|calendarizacao|live|evolution-cache)\.json$|^misscan\/report-snapshots\/\d{4}-\d{2}-\d{2}_\d{4}-\d{2}-\d{2}\.json$|^misscan\/history\/\d{4}-\d{2}\.json$|^misscan\/history-shards\/\d{4}-\d{2}-\d{2}\.json$|^misscan\/history-days\/\d{4}-\d{2}-\d{2}\.json$/;
let version = 0;
let runtimeEnv = {};
let runtimeOrigin = 'https://local.invalid';
let writeTablesReady = null;
...
function database() {
  return runtimeEnv?.MISSCAN_DB || null;
}

// As tabelas já são criadas no primeiro sync. Leituras não devem executar DDL:
// em um Worker frio, os dois CREATE TABLE IF NOT EXISTS adicionavam vários
// segundos a toda abertura do painel. Escritas continuam garantindo o schema.
async function ensureWriteTables() {
  const db = runtimeEnv?.MISSCAN_DB;
  if (!db) return null;
  if (!writeTablesReady) {
    writeTablesReady = (async () => {
      await db.prepare(`
      ...
      writeTablesReady = null;
      throw error;
    });
  }
  await writeTablesReady;
  return db;
}
...
// Revisão leve para o polling do navegador. Evita baixar e serializar HC,
// GEROT e metadados completos apenas para descobrir se algo mudou.
export async function readJsonRevision(pathname) {
  const db = database();
  if (db) {
    const row = await db.prepare(
      'SELECT updated_at FROM misscan_store WHERE key = ?1'
    ).bind(pathname).first();
...
export async function writeJson(pathname, data) {
  ...
  const now = new Date().toISOString();
  const db = await ensureWriteTables();
  if (db) {
...
export function isoWeekStartKey(dateKey) {
...
export function daysBetween(from, to, limit = 730) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(from) || !/^\d{4}-\d{2}-\d{2}$/.test(to)) return [];
...
async function mapHistoryLimit(items, limit, worker) {
...
export async function readHistoryRange(from, to, meta = {}) {
  ...
  if (meta?.storageGranularity === 'day-v1' || meta?.storageGranularity === 'hybrid-day-week-v1') {
    const dayStats = meta?.dayStats && typeof meta.dayStats === 'object'
      ? meta.dayStats
      : {};
    const availableDays = new Set(
      Array.isArray(meta?.days) ? meta.days : Object.keys(dayStats)
    );
    ...
    // D1 responde muito melhor quando os arquivos diários são lidos em pequenos
    // lotes paralelos. O limite evita sobrecarregar o Worker e preserva a ordem.
    const dailyRows = await mapHistoryLimit(storedDays, 6, async day => {
      const file = await readJson(`misscan/history-days/${day}.json`, { rows: [] });
      return (file?.rows || []).filter(row => rowDateKey(row) === day);
    });
    for (const dayRows of dailyRows) {
      rows.push(...dayRows);
    }

    // Durante a migração, dias ainda não reconstruídos continuam legíveis nos
    // blocos semanais antigos. Assim nenhum histórico é apagado antes do
    // backfill diário terminar.
    if (meta?.storageGranularity === 'hybrid-day-week-v1' && missingDays.length) {
      const legacyStats = meta?.legacyShardStats || {};
      const legacyShards = Array.isArray(meta?.legacyShards)
        ? meta.legacyShards
        : Object.keys(legacyStats);
      const missing = new Set(missingDays);

      for (const shard of [...new Set(legacyShards)].sort()) {
        if (!/^\d{4}-\d{2}-\d{2}$/.test(shard)) continue;
        const stats = legacyStats[shard] || {};
        const shardStart = String(stats.start || shard);
        const shardEnd = String(stats.end || '9999-12-31');
        if (shardEnd < start || shardStart > end) continue;
        const file = await readJson(`misscan/history-shards/${shard}.json`, { rows: [] });
        for (const row of (file?.rows || [])) {
          const date = rowDateKey(row);
          if (missing.has(date)) rows.push(row);
        }
      }
    }
  ...
  if (meta?.storageGranularity === 'iso-week-v1') {
    const shardStats = meta?.shardStats && typeof meta.shardStats === 'object'
      ? meta.shardStats
      : {};
    const shards = Array.isArray(meta?.shards)
      ? meta.shards
      : Object.keys(shardStats);

    for (const shard of [...new Set(shards)].sort()) {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(shard)) continue;
      const stats = shardStats[shard] || {};
      const shardStart = String(stats.start || shard);
      const shardEnd = String(stats.end || '9999-12-31');
      if (shardEnd < start || shardStart > end) continue;

      const file = await readJson(
        `misscan/history-shards/${shard}.json`,
        { rows: [] }
      );
      for (const row of (file?.rows || [])) {
        const date = rowDateKey(row);
        if (date && date >= start && date <= end) rows.push(row);
      }
    }
  ...
  // Compatibilidade de leitura com o histórico mensal anterior à V7.1.
  const indexed = new Set(Array.isArray(meta?.months) ? meta.months : []);
  for (const month of monthsBetween(start, end)) {
    if (indexed.size && !indexed.has(month)) continue;
