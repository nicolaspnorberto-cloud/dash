...
const V652_FORECAST_ORIGIN_TYPE = 'INTER-SOC';
...
function instalarAutomacoesV65() {
  ...
  Logger.log(
    'V6.5.2 instalada: HC+LM 30 min | GEROT Packed+Forecast INTER-SOC 30 min | E-mails 5 min. ' +
  ...
  try {
    ...
    const payload = {
      ...
      meta: {
        ...
        forecastOriginType: V652_FORECAST_ORIGIN_TYPE,
        site: V65_GEROT_SITE,
    ...
    Logger.log(
      'V6.5.2 GEROT OK: ' + data.processedRows + ' linhas Packed agregadas | ' +
...
function lerForecastGerotV65_(ss) {
  ...
  //
  // V6.5.2:
  // Para a projeção diária usamos SOMENTE origin_type = INTER-SOC.
  // Não somamos FMH, FULL, PUDO SVP, BIG SELLER ou outros perfis.
  ...
  values.forEach(function(row) {
    ...
    if (originType !== V652_FORECAST_ORIGIN_TYPE) return;
    ...
    // Normalmente existe uma linha INTER-SOC por dia.
    // O += mantém segurança caso a fonte tenha mais de uma linha INTER-SOC no mesmo dia.
    if (!map.has(date)) {
      map.set(date, {
        date: date,
        destination: V65_GEROT_SITE,
        originType: V652_FORECAST_ORIGIN_TYPE,
        week: week,
        total: 0,
        direct: 0,
        transhipment: 0
...
/**
 * V6.5.2 — teste objetivo do Forecast INTER-SOC.
...
function testarForecastInterSocV652(dataTeste) {
  ...
  Logger.log('Forecast INTER-SOC V6.5.2 para ' + dateKey + ':');
